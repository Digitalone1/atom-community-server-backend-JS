  ...
  try {
    ...
    const command = await sqlStorage`
      SELECT DISTINCT ON (p.name) p.name, v.semver, p.downloads,
        (p.stargazers_count + p.original_stargazers) AS stargazers_count, v.readme,
        JSONB_BUILD_OBJECT('type', v.repo_type, 'url', v.repo_url) AS repository, v.meta AS data
      FROM packages AS p
    ...
    const command = await sqlStorage`
      WITH search_query AS (
        SELECT DISTINCT ON (p.name) p.name, v.meta AS data, p.downloads,
          (p.stargazers_count + p.original_stargazers) AS stargazers_count, v.readme,
          JSONB_BUILD_OBJECT('type', v.repo_type, 'url', v.repo_url) AS repository,
  ...
  try {
    ...
    const command = await sqlStorage`
      WITH latest_versions AS (
        SELECT DISTINCT ON (p.name) p.name, v.meta AS data, p.downloads,
          (p.stargazers_count + p.original_stargazers) AS stargazers_count, v.readme,
          v.semver, JSONB_BUILD_OBJECT('type', v.repo_type, 'url', v.repo_url) AS repository,
          p.created, v.updated
        FROM packages AS p
          INNER JOIN versions AS v ON (p.pointer = v.package AND v.deleted IS FALSE
          ${
